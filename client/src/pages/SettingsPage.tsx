    ...
    <div>
      ...
      <div className="mt-1 flex gap-2">
        <div className="relative flex-1">
          <input type={show ? "text" : "password"} defaultValue={defaultValue} className="w-full rounded-md border border-border px-3 py-2 pr-10 text-sm outline-hidden focus:ring-2 focus:ring-primary/30" />
        ...
        <div className="rounded-lg border border-border bg-card p-5 card-shadow space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select defaultValue="IST" className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-hidden">
        ...
        <div className="rounded-lg border border-border bg-card p-5 card-shadow space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1 flex gap-2">
                <input type="text" readOnly value="https://api.flowdesk.app/webhook" className="flex-1 rounded-md border border-border bg-muted/30 px-3 py-2 text-sm text-muted-foreground outline-hidden" />
          ...
          {([
            ...
            <div key={n.key} className="flex items-center justify-between rounded-md border border-border p-4">
              ...
              <button onClick={() => handleToggleNotif(n.key)} className={`relative h-7 w-12 rounded-full transition-colors ${notifications[n.key] ? "bg-primary" : "bg-muted"}`}>
                <span className={`absolute top-0.5 h-6 w-6 rounded-full bg-card shadow-sm transition-transform ${notifications[n.key] ? "left-[22px]" : "left-0.5"}`} />
              </button>
            </div>
          ))}
        </div>
            ...
            {team.map((m) => {
              ...
              return (
                <div key={m.email} className="flex items-center gap-3 rounded-md border border-border p-3">
                  ...
                  {m.role !== "Owner" ? (
                    <select value={m.role} onChange={(e) => handleRoleChange(m.email, e.target.value)} className="rounded-md border border-border px-2 py-1 text-xs outline-hidden">
          ...
          <div className="w-full max-w-sm rounded-lg bg-card p-6 card-shadow modal-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="colleague@company.com" className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-hidden focus:ring-2 focus:ring-primary/30" />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground">Role</label>
                <select value={inviteRole} onChange={(e) => setInviteRole(e.target.value)} className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-hidden">
                  <option>Admin</option><option>Manager</option><option>Agent</option>
                </select>
              </div>
            </div>
