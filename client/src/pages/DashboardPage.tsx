  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.title} className={`rounded-lg border border-border border-l-4 ${stat.borderColor} bg-linear-to-br ${stat.gradient} p-5 card-shadow`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-xs p-4" onClick={() => setShowBotLogs(false)}>
          <div className="w-full max-w-2xl rounded-lg bg-card p-6 card-shadow modal-scale-in max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-2 rounded-md bg-muted px-3 py-2 mb-4">
              ...
              <input value={logSearch} onChange={(e) => setLogSearch(e.target.value)} placeholder="Search logs..." className="w-full bg-transparent text-sm outline-hidden placeholder:text-muted-foreground" />
            </div>
