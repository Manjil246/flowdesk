...
export default function AppSidebar({ mobileOpen, onMobileClose }: AppSidebarProps) {
  ...
  return (
    <>
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-foreground/40 backdrop-blur-xs lg:hidden" onClick={onMobileClose} />
      )}
      ...
      {/* Logout Modal */}
      {showLogout && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-foreground/40 backdrop-blur-xs p-4" onClick={() => setShowLogout(false)}>
