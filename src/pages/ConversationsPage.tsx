...
export default function ConversationsPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden">
      {/* Contact List */}
      <div className={`w-full flex-shrink-0 border-r border-border bg-card lg:w-[35%] ${showChat ? "hidden lg:flex" : "flex"} flex-col`}>
        <div className="border-b border-border p-3">
          <div className="flex items-center gap-2 rounded-lg bg-muted px-3 py-2">
            <Search className="h-4 w-4 text-muted-foreground" />
            <input type="text" placeholder="Search conversations..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground" />
          </div>
        </div>
          ...
          <>
            ...
            {showChatSearch && (
              <div className="flex items-center gap-2 border-b border-border bg-card px-4 py-2">
                <Search className="h-4 w-4 text-muted-foreground" />
                <input value={chatSearch} onChange={(e) => setChatSearch(e.target.value)} placeholder="Search in chat..." className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" autoFocus />
            ...
            <div className="flex-1 overflow-y-auto wa-chat-pattern px-4 py-4 custom-scrollbar">
              <div className="mb-4 flex justify-center">
                <span className="rounded-full bg-card/80 px-4 py-1 text-xs font-medium text-muted-foreground shadow-sm">Today</span>
              </div>
              <div className="space-y-2 max-w-3xl mx-auto">
                {filteredMessages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.sent ? "justify-end" : "justify-start"}`}>
                    <div className={`relative max-w-[75%] rounded-lg px-3 py-2 shadow-sm ${msg.sent ? "bg-wa-sent text-foreground bubble-sent" : "bg-wa-received text-foreground bubble-received"}`}>
