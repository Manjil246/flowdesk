    ...
    <div className="p-4 lg:p-6 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-border bg-card p-3">
        <div className="flex flex-1 items-center gap-2 rounded-md bg-muted px-3 py-2 min-w-[200px]">
          ...
          <input type="text" placeholder="Search by name, email, phone..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
        </div>
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setCurrentPage(1); }} className="rounded-md border border-border bg-card px-3 py-2 text-sm outline-none">
          <option>All</option><option>New</option><option>Contacted</option><option>Converted</option><option>Lost</option>
        </select>
        <select value={sourceFilter} onChange={(e) => { setSourceFilter(e.target.value); setCurrentPage(1); }} className="rounded-md border border-border bg-card px-3 py-2 text-sm outline-none">
      ...
      {/* Add/Edit Lead Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm p-4" onClick={() => { setShowAddModal(false); setEditLead(null); }}>
          <div className="w-full max-w-lg rounded-lg bg-card p-6 card-shadow modal-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="Enter full name" className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground">Phone Number <span className="text-destructive">*</span></label>
                <input type="text" value={formPhone} onChange={(e) => setFormPhone(e.target.value)} placeholder="+91 98765 43210" className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground">Email Address</label>
                <input type="email" value={formEmail} onChange={(e) => setFormEmail(e.target.value)} placeholder="email@example.com" className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-foreground">Source</label>
                  <select value={formSource} onChange={(e) => setFormSource(e.target.value as Lead["source"])} className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-none">
                ...
                <div>
                  ...
                  <select value={formStatus} onChange={(e) => setFormStatus(e.target.value as Lead["status"])} className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-none">
              ...
              <div>
                ...
                <textarea value={formNotes} onChange={(e) => setFormNotes(e.target.value)} placeholder="Add notes..." className="mt-1 w-full rounded-md border border-border px-3 py-2 text-sm outline-none" rows={2} />
              </div>
            </div>
          ...
          <div className="w-full max-w-lg rounded-lg bg-card p-6 card-shadow modal-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center border-b border-border pb-2">
                ...
                <select value={viewLead.status} onChange={(e) => handleViewLeadStatusChange(e.target.value as Lead["status"])} className="rounded-md border border-border px-2 py-1 text-xs outline-none">
                  <option>New</option><option>Contacted</option><option>Converted</option><option>Lost</option>
                </select>
              </div>
            ...
            <div className="mt-4">
              <label className="text-sm font-medium text-foreground">Notes</label>
              <textarea defaultValue={viewLead.notes} className="mt-1 w-full rounded-md border border-border bg-muted/30 px-3 py-2 text-sm outline-none" rows={3} placeholder="Add notes about this lead..." />
            </div>
      ...
      {showDeleteConfirm !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm p-4" onClick={() => setShowDeleteConfirm(null)}>
      ...
      {showBulkDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm p-4" onClick={() => setShowBulkDeleteConfirm(false)}>
          <div className="w-full max-w-sm rounded-lg bg-card p-6 card-shadow modal-scale-in" onClick={(e) => e.stopPropagation()}>
