...
export default function BotSettingsPage() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 max-w-4xl">
            ...
            <div className="flex-1 min-w-0">
              {s.editing ? (
                <div className="space-y-2">
                  <input value={s.title} onChange={(e) => updateStep(s.id, "title", e.target.value)} className="w-full rounded-md border border-border px-2 py-1 text-sm outline-none" placeholder="Step title" />
                  <input value={s.message} onChange={(e) => updateStep(s.id, "message", e.target.value)} className="w-full rounded-md border border-border px-2 py-1 text-sm outline-none" placeholder="Bot message" />
                </div>
              ) : (
                <>
              ...
              {rules.map((r, i) => (
                <tr key={i} className="border-b border-border/50">
                  <td className="px-3 py-2">
                    {r.editing ? (
                      <input value={r.keyword} onChange={(e) => updateRule(i, "keyword", e.target.value)} placeholder="keyword" className="w-full rounded-md border border-border px-2 py-1 text-xs outline-none" />
                    ) : (
                  ...
                  <td className="px-3 py-2">
                    {r.editing ? (
                      <input value={r.reply} onChange={(e) => updateRule(i, "reply", e.target.value)} placeholder="Reply message" className="w-full rounded-md border border-border px-2 py-1 text-xs outline-none" />
                    ) : (
      ...
      {/* Delete Step Confirm */}
      {showDeleteStep !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm p-4" onClick={() => setShowDeleteStep(null)}>
